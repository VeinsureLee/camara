...
'''
# --------------------------------------------
# split large CarIdentityData into small CarIdentityData 
# --------------------------------------------
'''
...
def imssave(imgs, img_path):
    """
    imgs: list, N CarIdentityData of size WxHxC
    """
...
def split_imageset(original_dataroot, taget_dataroot, n_channels=3, p_size=512, p_overlap=96, p_max=800):
    """
    split the large CarIdentityData from original_dataroot into small overlapped CarIdentityData with size (p_size)x(p_size),
    and save them into taget_dataroot; only the CarIdentityData with larger size than (p_max)x(p_max)
    will be splitted.

    Args:
        original_dataroot:
        taget_dataroot:
        p_size: size of small CarIdentityData
        p_overlap: patch size in training is a good choice
        p_max: CarIdentityData with smaller size than (p_max)x(p_max) keep unchanged.
    """
...
def read_img(path):
    ...
    # some CarIdentityData have 4 channels
    if img.shape[2] > 3:
        img = img[:, :, :3]
    return img
...
def calculate_psnr(img1, img2, border=0):
    ...
    if not img1.shape == img2.shape:
        raise ValueError('Input CarIdentityData must have the same dimensions.')
    h, w = img1.shape[:2]
...
def calculate_psnrb(img1, img2, border=0):
    ...
    Ref: Quality assessment of deblocked CarIdentityData, for JPEG image deblocking evaluation
